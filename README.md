...
- **What:** runs the trained model over the test records (entities live in the composed
  field-sentences; descriptions are O-context), parses predicted entities back into course
  fields, and measures field-level recovery vs ground truth (+ worked examples).
- **Why:** answers the coherence question — *is the pipeline wired so the business numbers are
  recomputed from model output rather than read from adjacent columns?* (It demonstrates the
  text → BIO → fields round trip, not extraction from free prose — see §11.)
...
The trained model parses the entities back out of the **composed field-sentences** and
reassembles them into structured fields. (Honest scope: the entities live in those composed
sentences; the free-text descriptions are O-context only — see §11. So this measures whether
structured fields survive the text → BIO → fields round trip, **not** extraction of fields
from real marketing prose.) Per-field recovery is bounded by the train-time label noise:

| Field | Recovery (NER-extracted == truth) |
| --- | --- |
| partner (institution) | _regenerated by `scripts/06`_ |
| category | _regenerated by `scripts/06`_ |
| duration | _regenerated by `scripts/06`_ |
| price | _regenerated by `scripts/06`_ |
| enrollment | _regenerated by `scripts/06`_ |
| level | _regenerated by `scripts/06`_ |

**Worked example** (text → extracted → truth): see the `examples` array in
[`results/extraction_bridge.json`](results/extraction_bridge.json) — each is a real record the
model was actually run on (the table above and this example are produced by `scripts/06`, not
hand-written).
...
that NER-reconstructed table**, then compared to the ground-truth-derived numbers. Agreement
shows the pipeline is wired end-to-end (the analytics run on model output, not adjacent
columns) — it does not show the model discovering fields absent from its training text:

| Business metric | From NER | From truth |
| --- | --- | --- |
| Paid-vs-free enrolment ratio | _regenerated by `scripts/06`_ | _regenerated by `scripts/06`_ |
| Top-demand category | _regenerated by `scripts/06`_ | _regenerated by `scripts/06`_ |
| Duration engagement peak | _regenerated by `scripts/06`_ | _regenerated by `scripts/06`_ |
